...
import { createTugasTL } from "@/actions/tl/create-tugas";
...
export function CreateTugasDialogTL({
  participants,
  division,
}: CreateTugasDialogProps) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");

  const [targetType, setTargetType] = useState<"division" | "individual">(
    "division"
  );
  const [selectedUserIds, setSelectedUserIds] = useState<string[]>([]);

  const divisionParticipants = useMemo(() => {
    return participants.filter((item) => item.division === division);
  }, [division, participants]);

  const selectedCount = useMemo(() => {
    if (targetType === "division") {
      return divisionParticipants.length;
    }

    return selectedUserIds.length;
  }, [divisionParticipants.length, selectedUserIds, targetType]);
  ...
  async function handleSubmit(
    ...
    try {
      ...
      const result = await createTugasTL({
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto rounded-[30px]">
        <DialogHeader>
          <DialogTitle>Buat Tugas Divisi {division}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setTargetType(
                    e.target.value as "division" | "individual"
                  )
                }
              ...
              >
                <option value="division">Semua Peserta Divisi Ini</option>
          ...
          {targetType === "individual" ? (
              ...
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Hanya peserta divisi {division} yang bisa dipilih.
                  </p>
                </div>
              ...
              <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
                {divisionParticipants.length > 0 ? (
                  ...
                  <div className="rounded-2xl bg-muted/40 p-4 text-sm text-muted-foreground">
                    Tidak ada peserta aktif di divisi ini.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-muted/30 p-4 text-sm text-muted-foreground">
              Tugas akan diberikan ke semua peserta divisi{" "}
